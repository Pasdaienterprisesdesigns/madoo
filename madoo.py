...
def fetch_recent_txs(limit_blocks=100):
    ...
    for i, blk in enumerate(range(latest_block, latest_block - limit_blocks, -1)):
        ...
        if i % 5 == 0:
            time.sleep(1)  # avoid hitting Etherscan rate limits
...
def run_dashboard():
    ...
    **4. MEV Bot Clusters** – Groups of transactions likely sent by the same bot (based on gas patterns).
    ...
    👉 Use the sidebar to set how many blocks you want to fetch (max 500 recommended for hosted apps).
    """)

    block_count = st.sidebar.slider("Number of Blocks to Fetch", 10, 1000, 50, step=10)
